...
func RegisterRoutes(
	...
	// REST API（需要认证）
	auth := r.Group("/api/v1", AuthMiddleware(agentRepo, jwtSecret))
	...
	llmAuth := AuthMiddleware(agentRepo, jwtSecret)
	registerLLMProxy(r, llmAuth, llmHandler)
...
func registerLLMProxy(r *gin.Engine, auth gin.HandlerFunc, h *llm.Handler) {
	// ── Anthropic API ─────────────────────────────────────────
	// POST /v1/messages              — 创建消息
	// POST /v1/messages/count_tokens — 计算 token 数量
	// POST /v1/messages/batches      — 创建批处理
	// GET  /v1/messages/batches/:id  — 查询批处理
	r.POST("/v1/messages", auth, h.ProxyAnthropic)
	r.POST("/v1/messages/*path", auth, h.ProxyAnthropic)
	r.GET("/v1/messages/*path", auth, h.ProxyAnthropic)
	...
	r.POST("/v1/chat/completions", auth, h.ProxyOpenAI)
	r.POST("/v1/completions", auth, h.ProxyOpenAI)
	r.POST("/v1/embeddings", auth, h.ProxyOpenAI)
	r.GET("/v1/models", auth, h.ProxyOpenAI)
	r.GET("/v1/models/*path", auth, h.ProxyOpenAI)
}
