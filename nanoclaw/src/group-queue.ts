...
export class GroupQueue {
  ...
  enqueueMessageCheck(groupJid: string): void {
    ...
    if (state.active) {
      ...
      // Don't close idle containers — the message loop will pipe
      // the message via IPC and the container will process it in place.
      // Closing here causes a race: the message loop writes IPC + advances
      // cursor, but the container exits before reading the file → message lost.
  ...
  sendMessage(groupJid: string, text: string): boolean {
    ...
    state.pendingMessages = false; // Message delivered via IPC, no need for new container
